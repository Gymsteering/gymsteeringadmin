...
class AddCustomerService {
    ...
    async addGym(data) {
        const { name, location, startDate, userId, userEmail } = data;

        if (!name || !location || !startDate || !userId || !userEmail) {
            return { success: false, message: "Please Provide All The Required Details", statusCode: 400 };
        }
        ...
        try {
            ...
            const [gym] = await db.query("INSERT INTO gym (name, location , startDate , ownerId) VALUES( ? , ? , ? , ?)", [name, location, startDate, owner[0].id]);
